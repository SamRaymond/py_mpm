            ...
            for di in ti.static(range(-2, 3)):
                for dj in ti.static(range(-2, 3)):
                    ...
                    if 0 <= cell_i < self.grid.node_count_x and 0 <= cell_j < self.grid.node_count_y:
                        ...
                        if self.grid.body_id[cell_i, cell_j][0] == -1:
                            self.grid.body_id[cell_i, cell_j][0] = mp_obj_id
                        elif self.grid.body_id[cell_i, cell_j][1] == -1 and self.grid.body_id[cell_i, cell_j][0] != mp_obj_id:
                            self.grid.body_id[cell_i, cell_j][1] = mp_obj_id

                        # Update node properties for center of mass
                        ti.atomic_add(self.grid.mass[cell_i, cell_j], mp_mass * shape_value)
                        ti.atomic_add(self.grid.momentum[cell_i, cell_j], mp_mass * self.particles.velocity[p_idx] * shape_value)

                        force = -(mp_mass / mp_density) * (
                            ti.Vector([grad_shape_x * shape_y, shape_x * grad_shape_y]) @ self.particles.stress[p_idx]
                        )
                        ti.atomic_add(self.grid.force[cell_i, cell_j], force)

                        # Update body1 properties
                        if self.grid.body_id[cell_i, cell_j][0] == mp_obj_id:
                            ...
                            ti.atomic_add(self.grid.force_body1[cell_i, cell_j], force)
                        ...
                        elif self.grid.body_id[cell_i, cell_j][1] == mp_obj_id:
                            ti.atomic_add(self.grid.mass_body2[cell_i, cell_j], mp_mass * shape_value)
                            ti.atomic_add(self.grid.momentum_body2[cell_i, cell_j], mp_mass * self.particles.velocity[p_idx] * shape_value)
                            ti.atomic_add(self.grid.force_body2[cell_i, cell_j], force)
                            volume_weight = self.particles.volume[p_idx] * ti.Vector([grad_shape_x * shape_y, shape_x * grad_shape_y])
    ...
    def update_grid(self):
        ...
        for i, j in ti.ndrange(self.grid.node_count_x, self.grid.node_count_y):
            if self.grid.mass[i, j] > 1e-9:
                ...
                # Contact Detection
                if self.grid.body_id[i, j][1] != -1:  # Two bodies detected at node
                    approach1 = 0.0
                    approach2 = 0.0
                    ...
                    if is_approach:
                        ...
                        if self.grid.mass_body1[i, j] != 0.0:
                            ...
                            approach = approach1
                            self.grid.velocity_body1[i, j] = (self.grid.momentum_body1[i, j] / self.grid.mass_body1[i, j]) - approach * contact_normals
                            self.grid.acceleration_body1[i, j] = (self.grid.force_body1[i, j] / self.grid.mass_body1[i, j]) - (approach * contact_normals) / dt

                            if self.time <= self.dt:
                                self.grid.velocity_body1[i, j] += self.grid.acceleration_body1[i, j] * 0.5 * dt
                            else:
                        ...
                        if self.grid.mass_body2[i, j] != 0.0:
                            ...
                            approach = approach2
                            self.grid.velocity_body2[i, j] = (self.grid.momentum_body2[i, j] / self.grid.mass_body2[i, j]) - approach * contact_normals
                            self.grid.acceleration_body2[i, j] = (self.grid.force_body2[i, j] / self.grid.mass_body2[i, j]) - (approach * contact_normals) / dt

                            if self.time <= self.dt:
                                self.grid.velocity_body2[i, j] += self.grid.acceleration_body2[i, j] * 0.5 * dt
                            else:
                                self.grid.velocity_body2[i, j] += self.grid.acceleration_body2[i, j] * dt
                ...
                for dj in ti.static(range(-2, 3)):
                    ...
                    if 0 <= cell_i < self.grid.node_count_x and 0 <= cell_j < self.grid.node_count_y:
                        ...
                        if node_mass > 0:
                            ...
                            node_obj_ids = self.grid.body_id[cell_i, cell_j]
                            if node_obj_ids[0] == mp_obj_id:
                                velocity = self.grid.velocity_body1[cell_i, cell_j]
                                acceleration = self.grid.acceleration_body1[cell_i, cell_j]
                                normals = self.grid.normals_body1[cell_i, cell_j]
                            elif node_obj_ids[1] == mp_obj_id:
                                velocity = self.grid.velocity_body2[cell_i, cell_j]
                                acceleration = self.grid.acceleration_body2[cell_i, cell_j]
                                normals = self.grid.normals_body2[cell_i, cell_j]
                            # else:
                            #     continue  # Skip if particle doesn't belong to either body at this node

                            velocity_update += velocity * shape_value
                            acceleration_update += acceleration * shape_value
                            normal_update += normals * shape_value
